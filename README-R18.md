...
`-fcompound-names` and `-findirect-choice` matter here: 3GPP modules reuse short
type names across nested contexts and rely on indirect CHOICE members, and the
generated code will not compile without them.
...
- **Some generated RRC code does not compile.** Of 2674 translation units generated
  from `rrc-rel18-v18_9.asn1`, 190 fail with unknown type names such as
  `SetupRelease_3430P180_t` — a forward-reference problem in the generated
  parameterized `SetupRelease` types.
